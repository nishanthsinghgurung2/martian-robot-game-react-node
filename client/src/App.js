  ...
  return (
    <div className="App">
      ...
      <form className="robots-position-form">
        <label
          className="robots-position-form-item"
          htmlFor="text-upper-coords"
        >
        ...
        {robotPositions.length > 0
          ? robotPositions.map((elem, index) => {
              const robotPositionId = `robot-${index}`;
              const robotInstructionId = `instruction-${index}`;
              return (
                <div
                  key={robotPositionId}
                  className="robot-position-instruction"
                >
                  <label
                    className="robots-position-form-item"
                    htmlFor={robotPositionId}
                  >
      ...
      <footer>
        <h3 className="footer-header" aria-label="Final robot coordinates">
          Final robot coordinates:
        </h3>
        {loading ? <div>Loading....</div> : null}
        {error ? (
          <div data-testid="server-error" className="error" aria-label={error}>
            {error}
          </div>
        ) : null}
