...
import {
  isRobotInstructionInvalid,
  isRobotPositionInvalid,
  isUpperCoordsInvalid,
} from './utils/utility';
...
const useRobotFinalPositionCalculate = () => {
  ...
  const updateRobotPosition = (e) => {
    ...
    updatedRobotPositions[e.target.dataset.idx][e.target.className] =
      e.target.value;
  ...
  const calculateFinalRobotPositions = async (e) => {
    ...
    try {
      const response = await fetch(
        'http://localhost:3001/get-final-robots-coords',
        {
          method: 'POST',
          mode: 'cors',
